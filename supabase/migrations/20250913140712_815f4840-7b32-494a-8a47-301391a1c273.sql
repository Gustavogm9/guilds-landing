-- Phase 5: Performance & Production (Fixed)
...
-- Create notification queue optimization indexes (regular indexes)
CREATE INDEX idx_project_email_notifications_processing 
ON public.project_email_notifications(status, retry_count, created_at)
WHERE status = 'pending';

CREATE INDEX idx_project_webhook_events_processing 
