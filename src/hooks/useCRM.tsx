...
export function useCRM(includeInactive: boolean = false) {
  ...
  const createRecurringActivity = useMutation({
    ...
    onSuccess: async () => {
      // Tentar gerar atividades (pode falhar sem impacto crítico)
      try {
        await supabase.functions.invoke('generate-recurring-activities');
      } catch (error) {
        // Log não-crítico para debug
        console.warn('⚠️ Edge function call failed (non-critical):', error);
        console.info('💡 Activities will be generated by scheduled cron job');
      }
      
      // Invalidar queries (apenas marcar como stale)
      ...
      // Refetch apenas queries ativas após delay (aguarda React estabilizar)
      setTimeout(() => {
      ...
      toast({
        ...
        description: "As próximas ocorrências serão geradas em breve.",
      });
    },
    onError: (error) => {
      toast({
